...
sub fnRunSysCMD  {
  ...
  if ($?) {
        ...
        #print "Info '@sys_cmd' command succed !\n";        
  }
...
sub fnAnalyzeDiffResults {
	...
	foreach my $s_curr_res_line (@l_cmd_results) {
		...
		if ($s_curr_res_line =~ /^diff \-\-git/) {
			...
			#if ( $s_curr_res_line =~ /^\+\+\+ \/dev\/null/) { $s_dev_null = 1;}			
			#if ( $s_curr_res_line =~ /^deleted file mode/) { $s_dev_null = 1;}			
			push(@l_last_diff_section,$s_curr_res_line);
		}
	}
	if (scalar(@l_last_diff_section) > 0 ) {
		if ($s_dev_null == 0 ) {
			...
			if (!($s_report_flag)) {
				...
				#if (-f $s_short_line) {
				#	printf $fh_merge_file "%-7s %-10s %-15s %-50s\n" ,"$s_orig_str" ,"local" , "manual_merge" ,"$s_short_line";
				#} else {
				#	printf $fh_merge_file "%-7s %-10s %-15s %-50s\n" ,"$s_orig_str" ,"not_local" , "auto_merge" ,"$s_short_line";
				#}
			}
		}
		#print "&&&&&&&&&\n";
...
sub fnCheckIfWeUnderUWA {
	...
	if ($i_under_project_dir == 0 ) {
		...
		#chdir("$s_project_dir_keep/$s_uwa_name_tmp");
	}
	#-----------------------------------
#	if (-f "./usr_setup_path" ) {
...
#		my $s_curr_pwd = `pwd`;
#		chomp($s_curr_pwd);
##		print "Info: source $s_curr_pwd/usr_setup_path\n";
#	} else {
#		print "\nWarning: you must run this script directly under project work area folder \!\n\n"; 
#		close(LOGFILE);
#		exit 0;	
#	}
...
sub fnAtGitAdd {
        ...
        #print "s_fileToAdd='$s_fileToAdd'\n";
        #print "b_allFiles='$b_allFiles'\n";

	if ($s_last_argv eq "-all") { 
		&ffnUsage;
		#$s_last_argv = ".";
		# check if we are not exist under root project
	        # work area , need to ignore this option to run
                # from this folder  $UWA_ROOT/$UWA_NAME
		#my $s_currDir = `pwd`;
	        #homp($s_currDir);
	        #f ($s_currDir eq "$ENV{UWA_ROOT}/$ENV{UWA_NAME}") {
		#print "\nWarning: you cannot run this command option directly on this folder \!\!\! \n\n"; 
		#close(LOGFILE);
		#exit 0;	
		#
	}
...
sub fnAtGitDiff { 
	  ...
	  if ($s_last_argv =~ /^\-/ ) { $s_last_argv = "";}
          ...
          #if ($b_origin )  { $s_cmd = $s_cmd . " HEAD...origin/master ";}
          if ($b_origin )  { 
		if (!(-e $s_last_argv)) {
	  ...
	  if ($b_short == 0 ) {	
		  ...
		  #print "$cmd | egrep -i '^(diff|deleted file mode)\n";
		  print "$cmd\n";
		  #@_ = `$cmd | egrep -i '^(diff|deleted file mode)' `;
		  @_ = `$cmd`;
...
#	  $s_last_argv =~ s/\*//;
#	  if (-d "$s_last_argv") {
#                  $cmd = "git ls-tree -r master --name-only \| grep $s_last_argv/.git_block_last_tag";
#	          $cmd =~ s/\/\//\//;
#                  my @s_status = `$cmd`;
#	          if (scalar(@s_status) > 0 ) { 
#			  $cmd = "git checkout $s_cmd $s_last_argv/.git_block_last_tag";
#			  $cmd =~ s/\/\//\//;
#			  print "$cmd\n";		
#			  fnRunSysCMD($cmd); 
##			  if ($s_tag_name eq "") {;# only for uwa_dep_rep  
#				if (-f "$s_last_argv/.git_block_last_tag") {
#					#`rm -f  $s_cmd $s_last_argv/.git_block_last_tag`;
#				}
#			  }
##		  } else {
#			  if ($s_tag_name eq "") {;# only for uwa_dep_rep  
...
#				}
#			  } else {
#				  $cmd = "git checkout --force $s_cmd $s_last_argv/.git_block_last_tag";
#				  $cmd =~ s/\/\//\//;
#				  print "$cmd\n";		
#				  fnRunSysCMD($cmd); 
#				  if ($s_tag_name eq "") {;# only for uwa_dep_rep  
...
#			  }
#		  }
#                  $cmd = "git ls-tree -r master --name-only | grep $s_last_argv/depends.list";
#	          $cmd =~ s/\/\//\//;
#                  @s_status = `$cmd`;
#	          if (scalar(@s_status) > 0 ) { 
#			  $cmd = "git checkout $s_cmd $s_last_argv/depends.list";
#			  $cmd =~ s/\/\//\//;
#			  print "$cmd\n";		
#			  fnRunSysCMD($cmd); 
#		  }
