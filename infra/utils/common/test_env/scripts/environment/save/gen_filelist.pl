...
sub fnSet_usr_setup_path {
	...
	#printf $fh "%-2s %-25s %-20s\n","setenv", "UWA_PROJECT_ROOT", "$s_uwa_root";
...
sub fn_GetAllChild_inDependsList {
	...
	while (my $row = <$fh_dep>) {
		...
		if (!(-f $sBlock_filelist)) {
			print LOGFILE "Info: no such file list '$sBlock_filelist' \n"; 
			next;
		}
...
#
	...
	$s_current_dir = `pwd`;
	chomp($s_current_dir);
	#$s_current_dir = "/space/users/$ENV{USER}/$ENV{PROJECT_NAME}\_$s_proj_rev";
