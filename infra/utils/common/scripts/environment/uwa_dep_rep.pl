...
use lib '/Users/aduvdevani/Winbond/project/infra/utils/common/test_env/scripts/environment/packages/';
use strict;
use warnings;
use Dump qw(dump);
use Carp;
...
#
	...
	printf "%-3s %-13s %-3s %-20s %-3s %-40s %-1s\n","|","master/branch","|","block_name","|","block_version","|"; 
	printf "%-1s %-13s %-1s %-20s %-1s %-40s %-1s\n","+","---------------","+","----------------------","+","------------------------------------------","+"; 
	...
	if (scalar(@l_all_dependList_inWA) == 0 ) {
		...
		while (my $f = readdir(D)) {
			...
			if (defined $ENV{$s_block_path})  {
				...
				#-----------------------------------------
				# check if block is under master or branch
				chdir($f);
				...
				if ($s_master_or_branch =~ /master/) { 
					...
					if ($s_b_version eq "latest") { $s_b_version = "";}
				...
				chdir($s_work_area_fullpath);
				#-----------------------------------------
				printf "%-3s %-13s %-3s %-20s %-3s %-40s %-1s\n","|","$s_master_or_branch","|","$f","|","$s_b_version","|"; 
			        printf "%-1s %-13s %-1s %-20s %-1s %-40s %-1s\n","+","---------------","+","----------------------","+","------------------------------------------","+"; 
				printf LOGFILE "%-3s %-13s %-3s %-20s %-3s %-40s %-1s\n","|","$s_master_or_branch","|","$f","|","$s_b_version","|"; 
			        printf LOGFILE "%-1s %-13s %-1s %-20s %-1s %-40s %-1s\n","+","---------------","+","----------------------","+","------------------------------------------","+"; 
			}
	...
	my @l_ref_blocks = `grep  \"_path\" $s_usr_setup_path_file \| grep -v \"\^\#\"`; 
	if (scalar(@l_ref_blocks) > 0 ) {
		...
		foreach my $s_ref_block_data (@l_ref_blocks) {
			...
			$s_b_name =~ s/_path//; 
			next if ( $s_b_version =~ /\/$sWorkArea_name\//);
