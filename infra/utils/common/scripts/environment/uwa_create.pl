...
#************************************************************************
#
use lib '/Users/aduvdevani/Winbond/project/infra/utils/common/scripts/environment/packages/';
use strict;
use warnings;
use Dump qw(dump);
use Cwd;
	    ...
	    foreach my $item ($copied_hash{$top_name}{$block_name}){
		      ...
		      if (!(-d "$sTmp_sDir_block_folder")) {
				`mkdir -p $sTmp_sDir_block_folder`;
                      ...
                      my $s_depends_file = "$sTmp_sDir_block_folder/lists/depends.list";
		      $s_depends_file =~ s/\/\//\//;
		      if (!(-f $s_depends_file)) {
...
sub fnGet_all_child_hier_depends_list {
	...
	if (-f "$sRlease_path/lists/depends.list") {
		my $sDep_filename = "$sRlease_path/lists/depends.list";
		...
		print "Info: no such depend list file - '$sRlease_path/lists/depends.list' \n";
	}
	...
	if (-f $filename) {;
		...
		foreach my $sOne_block_data (@l_all_hier_sub_blocks) {
			...
			if ( $sOne_block eq "$sTop_name") {
				$s_ref_top_dependList = "$sMain_release_path/$sOne_block/$sOne_block_ver/lists/depends.list";
			}
		}
		close $fh;
	}
...
#
	...
	$s_curr_space_dir = "$ENV{UWA_SPACE_ROOT}";
	$s_project_dir = "$ENV{UWA_PROJECT_ROOT}";
	if (!(-d "$s_project_dir")) {
	...
	if (!(-d "$sWorkArea_name/$sBlockCluster_name")) {
		...
		if (($sRevision_number ne "latest") && ($sRevision_number ne "git_head"))  {
			if ($b_branch) {;# checkot block tag as branch reference
				...
				if (!(-d "$sWorkArea_name")) {`mkdir $sWorkArea_name`;}
				...
				if ($b_debug) {print "Info: cmd 'git clone $ENV{GIT_PROJECT_ROOT}/$sBlockCluster_name/\.git $sWorkArea_name/$sBlockCluster_name'\n";}
