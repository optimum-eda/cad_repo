...
#---------------------------------------------------------------------------
	...
	my $s_curr_dir = `pwd`;
        chomp($s_curr_dir);
	#------------------------------------
	# check if block is comes from branch
	# if yes : cannot create new tag
	chdir("$s_work_area_fullpath/$i_sBlock_name");	
	my $s_master_or_branch = `git branch 2>&1`;
	chomp($s_master_or_branch);	
	if (!($s_master_or_branch =~ /master/)) { 
		$s_log_message = "Error: this block '$i_sBlock_name' checked out from branch ,you cannot create new tag on branch \!\!\!";
		fnPrintMessageOut($s_log_message);
	}
	...
	while( my($s_one_sub_block,$s_one_sub_block_ver,$s_sub_block_parent) = splice(@l_all_hier_sub_blocks_tmp,0,3)) {
		#next if ($s_one_sub_block eq "$i_sBlock_name");
		next if ($s_one_sub_block_ver eq "head");
		next if (($s_one_sub_block eq "$i_sBlock_name") && ($s_one_sub_block_ver eq "head"));
	...
	if ($i_bRelease) {
		...
		@l_all_hier_sub_blocks_tmp = @l_all_hier_sub_blocks;
		while( my($s_one_sub_block,$s_one_sub_block_ver,$s_sub_block_parent) = splice(@l_all_hier_sub_blocks_tmp,0,3)) {
			next if ($s_one_sub_block ne "$i_sBlock_name");
